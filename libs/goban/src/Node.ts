...
export class Node {
    ...
    constructor(board: Board, parent: Node | null, info?: NodeInfo) {
        this.board = board;
        this.parent = parent;
        this.children = [];
        this.changes = [];
    ...
    Revert() {
        for (let change of this.changes) {
            if (change.stone) {
                this.board.SetStone(change.coord.x, change.coord.y, change.oldStone ?? STONE.EMPTY);
            } else if (change.mark) {
                this.board.SetMark(change.coord.x, change.coord.y, change.oldMark ?? MARK.NONE);
            } else if (change.label) {
