...
export class Reader {
    ...
    constructor(record: _Record) {
        this.record = record;
    }

    Setup(node: Node, name: keyof typeof C.SETUP_PROPS, values: any): boolean {
        const setupMap = { 'AB': STONE.BLACK, 'AW': STONE.WHITE, 'AE': STONE.EMPTY } as unknown as Record<keyof typeof C.SETUP_PROPS, STONE>;
    ...
    Move(node: Node, key: string, values: string[], moveMarks?: boolean): boolean {
        const player = key == 'B' ? STONE.BLACK : STONE.WHITE;

        if (values[0].length != 2) {
            throw new Error("Wrong length in move values.")
        }

        let coord = Coordinate.FromSGF(values[0])
        ...
        if (player == STONE.BLACK) {
            node.info.blackCaptures += play.captures?.length ?? 0;
        } else {
            node.info.whiteCaptures += play.captures?.length ?? 0;
        }
    ...
    Mark(node: Node, name: keyof typeof C.MARKER_PROP_MAP, values: any) {
        const coords = this.ExplodeList(values);
    ...
    Info(node: Node, name: keyof typeof C.INFO_PROP_MAP, values: any) {
        var field = C.INFO_PROP_MAP[name];
        (node.info as any)[field] = values[0];
        return true;
    }
    ...
    HandleProp(node: Node, prop: string, values: string[], moveMarks: boolean) {
        if (C.SETUP_PROPS.indexOf(prop) > -1) {
            return this.Setup(node, prop as keyof typeof C.SETUP_PROPS, values);
            ...
            return this.Mark(node, prop as keyof typeof C.MARKER_PROP_MAP, values);
        } else if (C.INFO_PROPS.indexOf(prop) > -1) {
            return this.Info(node, prop as keyof typeof C.INFO_PROP_MAP, values);
        }
        return true;
    }
    ...
    static FromSGF(sgf: string, moveMarks?: boolean) {
        ...
        const record = new _Record(size);
        const reader = new Reader(record);

        if (!reader.RecurseRecord(data, moveMarks ?? false)) {
            return null;
        }
